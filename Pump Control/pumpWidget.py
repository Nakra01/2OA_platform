...
class PumpControl(QtWidgets.QWidget):
    def __init__(self, parent, pumpName:any):
        ...
        self.pumpModelCombo.addItems([ "", "MilliGAT LF", "MilliGAT HF", "Chemyx Nexus 4000", "Chemyx Fusion 6000X", "Chemyx Fusion 4000X"])
    ...
    def formatWidget(self, pump):
        if pump == "MilliGAT LF" or pump == "MilliGAT HF":
            self.resetWidget()
            self.pumpGroupBox.setMaximumHeight(300)
            self.pumpGroupBox.setMaximumWidth(200)
        ...
        if pumpModel == "MilliGAT HF":
            if self.pumpAddressText.text() == '':
                ...
                try:
                    ...
                    print("COM already connected")
                self.pumpObj = milliGAT_pump.Milligat(name=address, ser=self.ser)
            ...
            if self.pumpAddressText.text() == '':
                ...
                try:
                    ...
                    print("COM already connected")
                    self.ser = self.parent.pump1.ser
                self.pumpObj = milliGAT_pump.Milligat(name=address, ser=self.ser)
